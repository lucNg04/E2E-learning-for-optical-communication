...
def continuous_da_mod_torch(
    ...
    sig_n1, _, _ = c_da_mod_torch(seg1_norm, a1, b1, N1)
    sig_n2, _, _ = c_da_mod_torch(seg2_norm, a2, b2, N2)
...
def continuous_da_demod_torch(
    ...
    if scale1 is not None:
        rx_seg1 = rx_seg1 * scale1
    if scale2 is not None:
        rx_seg2 = rx_seg2 * scale2
...
#SNR
def normalize_unit_power(x: torch.Tensor, eps=1e-12):
    p = torch.mean(torch.abs(x) ** 2)
    return x / torch.sqrt(p + eps)

def real_mmse_gain(tx: torch.Tensor, rx: torch.Tensor, eps=1e-12):
    """
    Match baseline getN0_MMSE:
    minimize || h * tx - rx ||^2 over real scalar h
    """
    tx = normalize_unit_power(tx, eps)
    rx = normalize_unit_power(rx, eps)

    denom = torch.sum(torch.abs(tx) ** 2).real + eps
    numer = torch.real(torch.sum(torch.conj(tx) * rx))
    h = numer / denom
    return h, tx, rx
def snr_db_baseline_style(tx: torch.Tensor, rx: torch.Tensor, eps=1e-12):
    """
    More strictly aligned with fda_rof_funcs_v2.py:
      1) normalize tx/rx to unit average power
      2) find real scalar h minimizing ||h*tx - rx||^2
      3) compute N0 = (1 - h^2) / h^2
      4) SNR = 10 log10((1 - N0) / N0)
    """
    h, tx_n, rx_n = real_mmse_gain(tx, rx, eps=eps)

    # baseline-consistent N0
    h2 = h * h
    N0 = (1.0 - h2) / (h2 + eps)

    # numerical protection
    N0 = torch.clamp(N0, min=eps)
    one_minus_N0 = torch.clamp(1.0 - N0, min=eps)

    snr_db = 10.0 * torch.log10(one_minus_N0 / N0)

    err = h * tx_n - rx_n
    err_power = torch.mean(torch.abs(err) ** 2)

    aux = {
        "h": h.detach().item(),
        "N0": N0.detach().item(),
        "err_power": err_power.detach().item(),
    }
    return snr_db, aux
def compute_surrogate_loss(model, x, seg_weight=0.2):
    """
    x: complex tensor [L]
    """
    ...
    loss_total = complex_mse(x_hat, x_use)
    loss_seg1 = complex_mse(rx_seg1, aux["mod_aux"]["ofdm_seg1"])
    loss_seg2 = complex_mse(rx_seg2, aux["mod_aux"]["ofdm_seg2"])

    loss = loss_total + seg_weight * loss_seg1 + seg_weight * loss_seg2
...
def compute_surrogate_snr_loss(model, x, seg_weight=0.2):
    """
    x: complex tensor [L]
    objective: maximize total SNR, with seg1/seg2 auxiliary SNR terms
    """
    x_hat, rx_seg1, rx_seg2, aux = model(x)
    ...
    snr_total_db, aux_total = snr_db_baseline_style(x_use, x_hat)
    snr_seg1_db, aux_seg1 = snr_db_baseline_style(ref_seg1, rx_seg1)
    snr_seg2_db, aux_seg2 = snr_db_baseline_style(ref_seg2, rx_seg2)

    # maximize SNR  <=> minimize negative SNR
    loss = -(snr_total_db + seg_weight * snr_seg1_db + seg_weight * snr_seg2_db)
