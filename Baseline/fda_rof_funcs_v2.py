...
def Continous_DA_Mod(OFDM_Sig, para_N1, para_N2, Rt):
    ...
    if len(sig1) > 0 and np.max(np.abs(sig1)) > 0:
        ...
        scaling_factor_1 = np.max(np.abs(sig1))
    if len(sig2) > 0 and np.max(np.abs(sig2)) > 0:
        sig2 = sig2 / np.max(np.abs(sig2))
        scaling_factor_2 = np.max(np.abs(sig2))
    Sig_N1, _ = C_DA_Mod(sig1, para_N1["a"], para_N1["b"], N1)
    Sig_N2, _ = C_DA_Mod(sig2, para_N2["a"], para_N2["b"], N2)

    return Sig_N1.reshape(-1), Sig_N2.reshape(-1), OFDM_Segment1, OFDM_Segment2,scaling_factor_1,scaling_factor_2


def Continous_DA_DeMod(RX_Signal_N1, RX_Signal_N2, para_N1, para_N2, Rt,scaling_factor_1,scaling_factor_2):
    p = np.ceil(Rt) - Rt
    frac = Fraction(float(p)).limit_denominator(10**6)
    m, n = frac.numerator, frac.denominator
    ...
    RX_OFDM_Sig_Segment1 = C_DA_DeMod(RX_Signal_N1, para_N1["a"], para_N1["b"], N1).reshape(-1)*scaling_factor_1
    RX_OFDM_Sig_Segment2 = C_DA_DeMod(RX_Signal_N2, para_N2["a"], para_N2["b"], N2).reshape(-1)*scaling_factor_2
...
def TX(params):
    ...
    Sig_N1, Sig_N2, Signal_OFDM_Segment1, Signal_OFDM_Segment2,scaling_factor_1,scaling_factor_2 = Continous_DA_Mod(
...
def RX(TX_output, params):
    ...
    scaling_factor_1 = TX_output["scaling_factor_1"]
    scaling_factor_2 = TX_output["scaling_factor_2"]
    ...
    if B1 <= B2:
        ...
        Sys_Symbolrate = B1
    RxSignal = RxSignal / np.max(np.abs(RxSignal))
    G = 2e9
    f_inner = B1 / 2 + G
    f_outer = f_inner + B1 / 2 + B2 / 2 + G
    ...
    Rx_OFDM_Sig, RX_OFDM_Sig_Segment1, RX_OFDM_Sig_Segment2 = Continous_DA_DeMod(
        RX_Signal_N1, RX_Signal_N2, para_N1, para_N2, Rt,scaling_factor_1,scaling_factor_2
    )
