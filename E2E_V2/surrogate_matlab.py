...
def design_lowpass_fir(num_taps, cutoff, dtype=torch.float64, device=None):
    n = torch.arange(num_taps, dtype=dtype, device=device)
    m = (num_taps - 1) / 2
    h = cutoff * torch.sinc(cutoff * (n - m))
    w = torch.hamming_window(num_taps, periodic=False, dtype=dtype, device=device)
    h = h * w
    h = h / h.sum()
    return h

def complex_fir_conv1d(x, h):
    # x: [N] complex
    # h: [K] real
    xr = x.real.view(1, 1, -1)
    xi = x.imag.view(1, 1, -1)
    hh = h.flip(0).view(1, 1, -1)

    pad = h.numel() // 2
    xr = F.pad(xr, (pad, pad))
    xi = F.pad(xi, (pad, pad))

    yr = F.conv1d(xr, hh).view(-1)
    yi = F.conv1d(xi, hh).view(-1)
    return yr + 1j * yi

def upsample_1d(x, up):
    if up == 1:
        return x
    y = torch.zeros(x.numel() * up, dtype=x.dtype, device=x.device)
    y[::up] = x
    return y

def resample_fir_complex(x, up, down, num_taps=63):
    g = math.gcd(up, down)
    up //= g
    down //= g
    ...
    cutoff = 1.0 / max(up, down)   # normalized to Nyquist
    h = design_lowpass_fir(num_taps, cutoff, dtype=x.real.dtype, device=x.device)
    h = h * up

    xu = upsample_1d(x, up)
    yf = complex_fir_conv1d(xu, h)
    y = yf[::down]
    return y
